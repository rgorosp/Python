...
def montar_linhas_restart(
    ...
    Isso cobre:
    - arquivo criado cedo e lido muito no fim
    - recriações como G1, G1A, G1B...
    """
...
def transformar_jcl(
    ...
    """
    Regras consolidadas:
    - agrupa criações do mesmo step num mesmo símbolo base
    - suporta recriação do mesmo DSN (+2, +3...) em G1A, G1B...
    - troca e valida (+1), (+2), (&Gx)
    - se já existir //*R, substitui
    - se não existir //*R, insere automaticamente
    - se existir //*D, insere restart logo abaixo
    - se não existir //*D, insere antes do primeiro comando útil do step
    - PROC quebrada quando necessário
    - restart considera uso do step atual até o fim
    """
